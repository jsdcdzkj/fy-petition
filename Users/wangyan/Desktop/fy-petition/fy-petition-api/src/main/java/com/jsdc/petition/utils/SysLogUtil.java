...
import com.jsdc.petition.service.sys_.SpApilogInsertTemplate;
...
public class SysLogUtil {
	...
	/** 底层 Mapper 对象 */
//	static SysLogMapper sysLogMapper;
//	@Autowired
//	public void setSpApilogMapper(SysLogMapper sysLogMapper) {
//		SysLogUtil.sysLogMapper = sysLogMapper;
//	}
	...
	public void setSpApilogInsertTemplate(SpApilogInsertTemplate spApilogInsertTemplate) {
		SysLogUtil.spApilogInsertTemplate = spApilogInsertTemplate;
	}
	...
	public static void startRequest() {
    	...
    	a.setUserId(StpUtil.getLoginId(0L));
	 ...
	 * @param aj
	 */
	public static void endRequest(String reqParam,AjaxJson aj) {
		if(isWeb() == false)  {
			return;
		}
		...
		// 读取本次请求的 ApiLog 对象 
		HttpServletRequest request = SpringMVCUtil.getRequest();
		SysLog a = (SysLog)request.getAttribute(APILOG_OBJ_SAVE_KEY);
		if(a == null) {
		...
		try {
			...
			a.setResString(aj.toString());
			a.setEndTime(new Date());
			if(StrUtil.isNotEmpty(reqParam)){
				a.setReqParame(reqParam);
			}
