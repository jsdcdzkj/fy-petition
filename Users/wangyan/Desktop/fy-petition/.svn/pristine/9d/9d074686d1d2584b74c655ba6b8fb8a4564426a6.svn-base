...
import com.jsdc.petition.mapper.sys_.LabourMapper;
...
@Service
@Transactional
public class LabourService extends BaseService<Labour> {
     ...
     * @return
     */
    public Page<Labour> getPageList(LabourVo vo) {
        QueryWrapper<Labour> queryWrapper = new QueryWrapper<>();

        //testMapper.getEntityList(new Page<>(vo.getPageNo(), vo.getPageSize()),vo);
        return labourMapper.selectPage(new Page<>(vo.getPageNo(), vo.getPageSize()), queryWrapper);
     ...
     * @return
     */
    public List<Labour> getList(Labour entity) {
        QueryWrapper<Labour> queryWrapper = new QueryWrapper<>();
        return labourMapper.selectList(queryWrapper);
    }
     ...
     * @return
     */
    public AjaxJson saveOrUpdateLabour(Labour bean) {
        if (null == bean.getId()) {
